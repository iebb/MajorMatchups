...
const data = [
  ...
  [
    { id: 'A round.2', parents: ['A'] },
    { id: 'B round.2', parents: ['B'] },
    { id: 'D round.2', parents: ['D'] },
    { id: 'C round.2', parents: ['C'] },
  ],
]
...
const renderChart = (data, options={}) => {
  ...
  {tangleLayout.nodes.map(
    n => [
        <path className="selectable node" data-id={n.id} stroke="black" strokeWidth="8" d={`M${n.x} ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}`} />,
        <path className="node" stroke="white" strokeWidth="4" d={`M${n.x} ${n.y - n.height / 2} L${n.x} ${n.y + n.height / 2}`} />,
        <text className="selectable" data-id={n.id} x={n.x + 4} y={n.y - n.height / 2 - 4} stroke={background_color} strokeWidth="2">{n.id}</text>,
