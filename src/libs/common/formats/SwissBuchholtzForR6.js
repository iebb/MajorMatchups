...
import {copy, getStatus, getWinnerFromScore, getWinnerFromScoreGeneric} from '../common';

export function SwissBuchholtzForR6(fromStage, toStage) {
  const {state} = this;
    ...
    const dfs = (p, m, mref, pool) => {
      ...
      for (let c = team2cands.length - 1; c >= 0; c -= 1) {
        ...
        if (`${team1.code}-${team2.code}` + suffix in gamescores) {
          ...
          const [winner, maxW] = getWinnerFromScoreGeneric(gs);
          if (winner) {
            picked = winner > 0 ? 1 : -1;
            if (
          ...
          if (tb) {
            ...
            if (
              ...
              if (`${t1.code}-${t2.code}#1` in gamescores) {
                ...
                const [winner, ] = getWinnerFromScoreGeneric(gs);
